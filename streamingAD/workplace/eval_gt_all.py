...
_ANNOTATION_CSV = PROJECT_ROOT / "datasets" / "cmdad" / "AutoAD-Zero" / "resources" / "annotations" / "cmdad_anno_with_face_0.2_0.4.csv"
_CHARBANK_JSON = PROJECT_ROOT / "datasets" / "cmdad" / "AutoAD-Zero" / "resources" / "charbanks" / "cmdad_charbank.json"


def _load_cmdad_annotation_gt() -> Optional[Dict[str, List[str]]]:
    """Load ground truth characters from CMDAD annotation pred_ids.

    Returns cmd_filename -> [role_name, ...] or None.
    """
    if not _ANNOTATION_CSV.is_file() or not _CHARBANK_JSON.is_file():
        return None

    actor_to_role: Dict[str, str] = {}
    with open(_CHARBANK_JSON, encoding="utf-8") as f:
        charbank = json.load(f)
    for _imdbid, actors in charbank.items():
        for a in actors:
            actor_to_role[a["id"]] = a.get("role", "")

    gt_map: Dict[str, List[str]] = {}
    with open(_ANNOTATION_CSV, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            cmd_fn = row.get("cmd_filename", "")
            pred_ids = row.get("pred_ids", "")
            chars: set = set()
            if pred_ids and pred_ids not in ("[]", "{}", "''"):
                try:
                    for aid in eval(pred_ids):
                        role = actor_to_role.get(aid, "")
                        if role:
                            chars.add(role)
                except Exception:
                    pass
            if chars:
                gt_map[cmd_fn] = list(chars)
    return gt_map if gt_map else None


def compute_critic(entries: list, annotation_gt_map: Optional[Dict[str, List[str]]] = None) -> float:
    """Compute CRITIC: character co-reference accuracy.

    If annotation_gt_map is provided, it overwrites entry.characters
    with ground truth from CMDAD annotation pred_ids.
    """
    ...
    ad_entries = []
    for e in entries:
        chars = [str(c).strip() for c in e.get("characters", []) if str(c).strip()]
        # Use annotation GT if available
        if annotation_gt_map:
            cmd_fn = str(e.get("cmd_filename", ""))
            gt_chars = annotation_gt_map.get(cmd_fn, [])
            if gt_chars:
                chars = gt_chars
...
def evaluate_movie(
    ...
    try:
        annotation_gt_map = _load_cmdad_annotation_gt()
        if annotation_gt_map:
            print(f"    Loaded CMDAD annotation GT: {len(annotation_gt_map)} entries")
        else:
            print("    No CMDAD annotation GT found, using entry.characters")
        critic_score = compute_critic(entries, annotation_gt_map=annotation_gt_map)
        result["CRITIC"] = round(critic_score, 4)
