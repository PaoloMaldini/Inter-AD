...
def _build_ad_entries(
    raw_entries: List[Dict[str, Any]],
    annotation_gt_map: Optional[Dict[str, List[str]]] = None,
) -> List:
...
def main():
    ...
    # Load CMDAD annotation GT for CRITIC metric
    annotation_gt_map: Optional[Dict[str, List[str]]] = None
    try:
        _anno_csv = PROJECT_ROOT / "datasets" / "cmdad" / "AutoAD-Zero" / "resources" / "annotations" / "cmdad_anno_with_face_0.2_0.4.csv"
        _charbank = PROJECT_ROOT / "datasets" / "cmdad" / "AutoAD-Zero" / "resources" / "charbanks" / "cmdad_charbank.json"
        if _anno_csv.is_file() and _charbank.is_file():
            import csv as _csv
            _actor_map: Dict[str, str] = {}
            with open(_charbank, encoding="utf-8") as _f:
                for _imdbid, _actors in json.load(_f).items():
                    for _a in _actors:
                        _actor_map[_a["id"]] = _a.get("role", "")
            _gt: Dict[str, List[str]] = {}
            with open(_anno_csv, encoding="utf-8") as _f:
                for _row in _csv.DictReader(_f):
                    _cmd_fn = _row.get("cmd_filename", "")
                    _pred_ids = _row.get("pred_ids", "")
                    _chars: set = set()
                    if _pred_ids and _pred_ids not in ("[]", "{}", "''"):
                        try:
                            for _aid in eval(_pred_ids):
                                _role = _actor_map.get(_aid, "")
                                if _role:
                                    _chars.add(_role)
                        except Exception:
                            pass
                    if _chars:
                        _gt[_cmd_fn] = list(_chars)
            if _gt:
                annotation_gt_map = _gt
    ...
    for idx, ad_file in enumerate(ad_files):
        ...
        # ── Traditional metrics ──
        t0 = time.time()
        ad_entries = _build_ad_entries(raw_entries, annotation_gt_map=annotation_gt_map)
